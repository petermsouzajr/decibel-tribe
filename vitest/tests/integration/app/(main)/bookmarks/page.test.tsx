...
// Mock useInfiniteQuery to simulate a successful fetch with no bookmarks
vi.mock("@tanstack/react-query", async (importOriginal) => {
  const actual = await importOriginal<typeof import("@tanstack/react-query")>();
  return {
    ...actual,
    useInfiniteQuery: vi.fn().mockReturnValue({
      status: "success", // Force success state
      data: { pages: [{ posts: [], nextCursor: null }], pageParams: [null] }, // Empty data
      fetchNextPage: vi.fn(),
      hasNextPage: false,
      isFetching: false,
      isFetchingNextPage: false,
      // Add other properties returned by useInfiniteQuery if needed by the component
    }),
  };
});
...
describe("Bookmarks Page", () => {
  ...
  it("should display a message when there are no bookmarks", () => {
    ...
    const message = screen.getByText("You don't have any bookmarks yet.");
