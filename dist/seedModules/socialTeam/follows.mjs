...
export async function seedFollows(prismaClient, createdUsers) {
    if (!prismaClient) {
    ...
    const followerData = []; // Use correct type
    const createdFollowsForReturn = [];
    // Filter users who can be followed (e.g., verified and not 'noFollowers')
    const followableUsers = createdUsers.filter((user) => user.isVerified && !user.username.includes("noFollowers"));
