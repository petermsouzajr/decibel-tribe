...
export const seedBookmarks = async (prisma, createdUsers, createdPosts) => {
    ...
    const allBookmarks = [];
    const eligibleUsers = createdUsers.filter((u) => !u.username.includes("noBookmarks"));
    ...
    for (let i = 0; i < createdPosts.length; i += 2) {
        ...
        const potentialBookmarkers = eligibleUsers.filter((u) => u.id !== post.userId);
        if (potentialBookmarkers.length === 0)
            continue; // Skip if only creator is eligible
