...
let streamChatClientInstance = null;
if (streamKey && streamSecret) {
    streamChatClientInstance = StreamChat.getInstance(streamKey, streamSecret);
}
else {
    console.warn("Stream Chat key or secret not found in environment variables. Stream Chat client not initialized.");
}
...
// --- Helper Functions ---
// Random Helpers
export const random = (min, max) => faker.number.int({ min, max });
export const weightedRandom = (base, factor = 1) => {
    return Math.floor(base * factor * faker.number.float({ min: 0.5, max: 1.5 }));
};
export const proportionateRandom = (users, factor) => {
    return random(Math.ceil(users * factor * 0.5), Math.ceil(users * factor * 1.5));
};
// AccountData Generator
export const accountDataGenerator = (value, users, factor) => {
    if (value === "random") {
