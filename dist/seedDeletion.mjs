...
export async function deleteTestUsers(prismaClient) {
    if (!prismaClient) {
    ...
    const partialUsernames = userTypes.map((userType) => `testUser${userType.charAt(0).toUpperCase() + userType.slice(1)}`);
    let userIds = [];
    try {
        ...
        console.log(`...${userIds.length} test users and related data deleted successfully from DB!`);
    }
    catch (error) {
...
export async function deleteTestUsersFromStreamChat(streamClient, testUserIds) {
    // Use the passed streamClient
    if (!streamClient) {
        console.warn("Stream Chat client is not available. Skipping Stream Chat user deletion.");
        return;
    }
    ...
    try {
        ...
        console.log(`...Found ${streamUsers.users.length} test users in StreamChat to delete.`);
        ...
        for (const user of streamUsers.users) {
            try {
                ...
                if (error instanceof Error) {
                    console.error(`--- Failed to delete test user ${user.id} from StreamChat:`, error.message);
                }
                else {
                    console.error(`--- Failed to delete test user ${user.id} from StreamChat:`, error);
                }
            }
        }
        console.log(`...Finished deleting ${deletedCount} test users from StreamChat.`);
    }
    catch (error) {
        if (error instanceof Error) {
            console.error("Error during StreamChat test user deletion query:", error.message);
        }
        else {
