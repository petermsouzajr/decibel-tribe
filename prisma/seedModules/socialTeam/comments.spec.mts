...
// Restore top-level vi.mock for seedUtils.mjs
vi.mock("../../seedUtils.mjs", async (importOriginal) => {
  const original = await importOriginal<typeof import("../../seedUtils.mjs")>();
  return {
    ...original,
...
describe("SocialTeam - seedPublicComments Module", () => {
  ...
  let seedPublicComments: (typeof import("./comments.mjs"))["seedPublicComments"];
  // Get the type for the *actual* generateId function if needed for casting
  let generateIdFromEntropySizeType: (typeof import("../../seedUtils.mjs"))["generateIdFromEntropySize"];

  beforeAll(async () => {
    ({ seedPublicComments } = await import("./comments.mjs"));
    // Get the actual type after mocks are set up
    const utils = await import("../../seedUtils.mjs");
