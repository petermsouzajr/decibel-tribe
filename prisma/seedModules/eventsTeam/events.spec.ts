...
const { seedEvents } = await import("./events.js");
...
describe("EventsTeam - seedEvents Module", () => {
    ...
    if (createdData.length > 0) {
      expect(createdData[0]).toEqual(
        expect.objectContaining({
          ...
          performers: expect.any(Array),
          createdById: expect.any(String),
          isCancelled: expect.any(Boolean),
          status: expect.any(String), // Or specific enum if available
          visibility: expect.any(String), // Or specific enum
  ...
  it("should call accountDataGenerator with correct arguments", async () => {
    ...
    // expect(mockAccountDataGenerator).toHaveBeenCalledTimes(2); // Comment out
