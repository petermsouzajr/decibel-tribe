...
const { prisma } = await import("../../seedUtils.mjs");
const { seedGroupMembers } = await import("./groupMembers.mjs"); // Add .mjs extension
...
describe("GroupsTeam - seedGroupMembers Module", () => {
  ...
  it("should return created group member data", async () => {
    ...
    // Based on mocks: 2 groups, owner + 2 members each (excluding owners/ineligible), so should be 2 owners + 4 members = 6?
    // Let's adjust mock to return 1 member per group for simplicity (total = 2 owners + 2 members = 4)
    mockAccountDataGenerator.mockReturnValue(1);
    const resultAfterMockChange = await seedGroupMembers(
      mockPrismaClient as any,
      mockUsers,
      mockGroups,
    );

    expect(resultAfterMockChange.length).toBe(4);
    expect(resultAfterMockChange[0]).toEqual({
    ...
    expect(resultAfterMockChange[2]).toEqual(
