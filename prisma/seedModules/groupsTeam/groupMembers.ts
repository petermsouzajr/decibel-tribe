...
export interface CreatedGroupMember {
  id: string;
  userId: string;
  groupId: string;
  role: GroupRole;
...
export async function seedGroupMembers(
  ...
  const membersData: Prisma.GroupMemberCreateManyInput[] = [];
  const createdMembersForReturn: CreatedGroupMember[] = [];
  ...
  for (const group of createdGroups) {
    ...
    for (let i = 0; i < memberQuantity && i < potentialMembers.length; i++) {
      ...
      const memberInput: Prisma.GroupMemberCreateManyInput = {
        id: memberId,
