...
vi.mock("../../seedUtils.mjs", () => ({
  faker: {
    ...
    string: { alphanumeric: mockFakerStringAlphanumeric },
  },
}));
...
const { seedMedia } = await import("./media.mjs");
...
describe("MediaTeam - seedMedia Module", () => {
  ...
  it("should generate correct URLs for IMAGE and VIDEO types", async () => {
    ...
    expect(createdData[0].url).toContain("picsum.photos"); // post1, item 1 (IMAGE)
    expect(createdData[1].url).toContain("BigBuckBunny.mp4"); // post1, item 2 (VIDEO)
    expect(createdData[1].type).toBe(MediaType.VIDEO);
    expect(createdData[2].url).toContain("picsum.photos"); // post3, item 1 (IMAGE)
  });
  ...
  it("should not call createMany if number of media per post is 0", async () => {
    mockFakerNumberInt.mockReturnValue(0); // 0 media items
    await seedMedia(mockPrismaClient, mockPosts);
    expect(mockPrismaClient.media.createMany).not.toHaveBeenCalled();
  });
