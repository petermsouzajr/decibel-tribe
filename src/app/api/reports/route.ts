...
export async function POST(req: NextRequest) {
  try {
    ...
    if (lastRecentReport) {
      const secondsSince = Math.floor((Date.now() - new Date(lastRecentReport.createdAt).getTime()) / 1000);
      const waitSeconds = Math.max(0, 60 - secondsSince);
      return NextResponse.json({ error: `Please wait ${waitSeconds}s before submitting another report.` }, { status: 429 });
    }
    ...
    try {
      switch (type) {
        case "post": {
          const exists = await (prisma as any).post.findUnique({ where: { id: targetId }, select: { id: true } });
          if (!exists) return NextResponse.json({ error: "Post not found" }, { status: 404 });
          break;
        }
        case "comment": {
          const exists = await (prisma as any).comment.findUnique({ where: { id: targetId }, select: { id: true } });
          if (!exists) return NextResponse.json({ error: "Comment not found" }, { status: 404 });
          break;
        }
        case "profile": {
          const exists = await (prisma as any).user.findUnique({ where: { id: targetId }, select: { id: true } });
          if (!exists) return NextResponse.json({ error: "User not found" }, { status: 404 });
          break;
        }
        case "group": {
          const exists = await (prisma as any).group.findUnique({ where: { id: targetId }, select: { id: true } });
          if (!exists) return NextResponse.json({ error: "Group not found" }, { status: 404 });
          break;
        }
        case "event": {
          const exists = await (prisma as any).event.findUnique({ where: { id: targetId }, select: { id: true } });
      ...
      return NextResponse.json({ error: "Failed to validate target" }, { status: 500 });
    }
    ...
    if (data.reportedId) duplicateWhere.OR.push({ reportedId: data.reportedId });
    ...
    if (duplicateWhere.OR.length > 0) {
      duplicateWhere.status = { in: [ReportStatus.PENDING, ReportStatus.INVESTIGATING] } as any;
      try {
        const existing = await (prisma as any).report.findFirst({ where: duplicateWhere, select: { id: true, createdAt: true } });
        if (existing) {
          return NextResponse.json({ error: "You have already reported this item and it is being reviewed." }, { status: 409 });
        }
      } catch (e: any) {
        // Fallback for environments where Prisma Client hasn't been regenerated yet
        const msg = String(e?.message || e);
        if (msg.includes("Unknown argument`)" ) || msg.includes("Unknown argument `commentId")) {
          console.warn("Duplicate-check fallback: Prisma Client not updated; continuing without commentId filter");
        } else {
          throw e;
        }
      }
    ...
    try {
      ...
      if (msg.includes("Unknown argument `commentId")) {
        console.warn("Create-report fallback: Prisma Client not updated; inserting with messageId instead of commentId");
        ...
        if (fallbackData.commentId) {
          ...
          fallbackData.adminNotes = (fallbackData.adminNotes ? fallbackData.adminNotes + " | " : "") + "fallback: original commentId stored in messageId";
        }
        const created = await (prisma as any).report.create({ data: fallbackData });
        return NextResponse.json(created, { status: 201 });
      }
      throw e;
    }
    ...
    return NextResponse.json(
      { error: "Internal server error", details: isDev ? String(error?.message ?? error) : undefined },
      { status: 500 },
    );
  }
}
