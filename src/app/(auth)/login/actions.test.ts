...
// Define explicit mocks for lucia methods
...
vi.mock("@/auth", async (importOriginal) => {
  ...
  return {
    ...
    lucia: {
      ...
      // Include other necessary properties/methods from the original lucia object if needed
      // e.g., readSessionCookie: actual.lucia.readSessionCookie
    },
    // Mock validateRequest if it's used indirectly
    validateRequest: vi.fn().mockResolvedValue({ user: null, session: null }),
  };
});

// Mock prisma user model with explicit mock functions
...
// Mock bcryptjs with explicit mock function
...
// This is often tricky. A simple function mock might suffice if only `set` is used.
const mockSetCookie = vi.fn();
vi.mock("next/headers", () => ({
  cookies: () => ({
    set: mockSetCookie,
    // Add other cookie methods like get, has, delete if needed
  }),
}));
...
// Mock resendVerificationEmail action
const mockResendVerification = vi.fn();
vi.mock("../sendVerification", () => ({
  resendVerificationEmail: mockResendVerification,
}));

// Dynamically import the module under test AFTER mocks
const { resendVerificationEmail } = await import("../sendVerification");
